          ...
          <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              ...
              <p className="text-slate-400 text-sm mt-0.5">
                {compareMode
                  ? 'Compare mode: click states to add them, then hit Compare'
            ...
            >
              ...
              {compareMode ? 'Cancel Compare' : 'Compare States'}
            </button>
          </div>
        </header>
        ...
        <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-6">
          <div className="bg-white rounded-2xl shadow-md overflow-hidden">
            <div className="px-6 pt-5 pb-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
              <div>
                <h2 className="text-lg font-semibold text-gray-800">National Retail Gas Prices</h2>
                {weekStart && (
                  <p className="text-xs text-gray-500 mt-0.5">Price as of {weekStart}</p>
                )}
              </div>
            </div>
            ...
            {compareMode && pendingCompare.size > 0 && (
              <div className="border-t border-amber-100 bg-amber-50 px-6 py-3 flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-sm font-semibold bg-amber-500 text-white hover:bg-amber-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  Compare {pendingCompare.size} states →
                </button>
              </div>
            )}
              ...
              <div className="border-t border-gray-100 px-6 py-4 bg-gray-50 flex items-center justify-center gap-3">
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    <p className="text-xs uppercase tracking-widest text-gray-500 font-medium">National Average</p>
                  ...
                  <p className="hidden sm:block text-sm text-gray-500 max-w-xs">
                    Click any state for its price history, or use Compare States to overlay multiple states.
                  </p>
                </div>
              </div>
            )}
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <header className="bg-slate-900 text-white px-6 py-4 shadow">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              {isCompareView ? (
                <>
                  <h1 className="text-xl font-bold">State Comparison</h1>
                  <p className="text-slate-400 text-xs mt-0.5">
                    Regular grade · {period} · {rangeLabel(range)}
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-slate-400 text-xs mt-0.5">
                    ...
                    {stateSource?.source === 'region' && stateSource?.region_name && (
                      <> · <span className="text-amber-400">Regional avg — {stateSource.region_name}</span></>
                    )}
                  </p>
                </>
              )}
        ...
        {/* State chips (compare view only) */}
        {isCompareView && (
          <div className="max-w-6xl mx-auto mt-3 flex flex-wrap gap-2">
            {selectedStates.map((abbr, i) => (
              <span
                key={abbr}
                className="flex items-center gap-1.5 pl-3 pr-2 py-1 rounded-full text-xs font-semibold text-white"
                style={{ backgroundColor: STATE_COLORS[i % STATE_COLORS.length] }}
              >
                {STATE_NAMES[abbr] || abbr}
                <button
                  onClick={() => handleRemoveState(abbr)}
                  className="w-4 h-4 flex items-center justify-center rounded-full hover:bg-white/20 transition-colors text-xs leading-none"
                  title={`Remove ${abbr}`}
                >
                  ×
                </button>
          ...
          {!isCompareView && statesData[firstState]?.length > 0 && (() => {
            ...
            return (
              <div className="flex flex-wrap gap-6 mb-6 pb-6 border-b border-gray-100">
                <div>
                  <p className="text-xs uppercase tracking-widest text-gray-500 font-medium">Latest Price</p>
                  <p className="text-3xl font-bold text-slate-800 mt-0.5">
                ...
                <div>
                  <p className="text-xs uppercase tracking-widest text-gray-500 font-medium">
                    Change ({rangeLabel(range)})
                  </p>
                  <p className={`text-2xl font-semibold mt-0.5 ${isUp ? 'text-red-600' : 'text-blue-700'}`}>
                    {isUp ? '▲' : '▼'} {formatPrice(Math.abs(change))}
                  </p>
                  <p className="text-xs text-gray-400 mt-0.5">from {oldest.date}</p>
                </div>
                <div className="h-12 w-px bg-gray-200 self-center hidden sm:block" />
                <div>
                  <p className="text-xs uppercase tracking-widest text-gray-500 font-medium">National Avg (latest)</p>
